...
class GameInfoAndAllOddsSequence(AbstractOddsFetcher):
    ...
    def get_odds(self, gid):
        ...
        for data_row in data_rows:
            ...
            try:
                odds_at_each_tick = re.findall('"' + matching_id + '\^(.+?)"', all_odds_data)[0].split(';')
            except IndexError:
                raise StopIteration

            for odds_tick in odds_at_each_tick[:-1]:
                tmp_array = odds_tick.split('|')
                tmp_timestamp = self._get_timestamp_from_string(gid, tmp_array[2])
                odds[bookie_name][tmp_timestamp] = {}
                odds[bookie_name][tmp_timestamp]["1"] = tmp_array[0]
                odds[bookie_name][tmp_timestamp]["2"] = tmp_array[1]

                probability[bookie_name][tmp_timestamp] = {}
                implied_prob_1 = 1/float(tmp_array[0])
                implied_prob_2 = 1/float(tmp_array[1])
