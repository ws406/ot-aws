...
def calculate_pre_commission_pnl(favTeamOdds, predict, result, goalDiff, dnbOdds, dcOdds):
    if favTeamOdds <= 1.83:
        if predict == result:
...
with open(file_name) as json_file:
    ...
    for match in matches:
        ...
        if "disqualified" in predict:
            ...
            local_profit = calculate_pre_commission_pnl(match['odds']['bet365']['final']['1'], '1', match['result'], match['home_score'] - match['away_score'], home_dnb_odds, home_dc_odds * 0.95)
            ...
            local_profit = calculate_pre_commission_pnl(match['odds']['bet365']['final']['2'], '2', match['result'], match['away_score'] - match['home_score'], away_dnb_odds, away_dc_odds * 0.95)
            profit = profit + local_profit
            if local_profit > 0:
                right = right + 1
                predict_result = 'right'
