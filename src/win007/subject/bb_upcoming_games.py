from src.win007.modules.games_fetcher.bb_games_fetcher import GamesFetcher
...
class Subject(SubjectInterface):
    ...
    def get_games(self, minutes, league_names):
        if league_names is None:
            games = self.games_fetcher.get_games_by_kickoff(minutes)
        else:
            games = self.games_fetcher.get_games_by_kickoff_and_league(minutes, league_names)
        return games
